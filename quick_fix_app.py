...
# Title
st.title("🎙️ WhisperForge Pipeline Demo")
st.markdown("### Audio to Content Generation Pipeline")
...
with st.sidebar:
    ...
    # After Pipeline options
    st.subheader("Editorial Enhancement")
...
async def run_pipeline(transcript, pipeline_type, provider, model, knowledge_base=None, enable_editor=True, editor_targets=None, generate_images=True, progress_callback=None):
    ...
    # Track step timing
    async def run_step(step_name, step_function, *args):
        step_start = time.time()
        try:
            result = await step_function(*args)
            step_duration = time.time() - step_start
            status = "success" if result and not (isinstance(result, str) and result.startswith("Error")) else "error"
                ...
                if "metrics" in results:
                    ...
                    for step_name, step_metrics in results["metrics"]["step_metrics"].items():
                        metrics_data.append({
                            ...
                            "Status": "❌ Error" if step_metrics.get("status") == "error" else "✅ Success"
                        })
...
# Modified pipeline orchestrator with progress reporting
async def run_pipeline_with_progress(transcript, pipeline_type, provider, model, knowledge_base=None, enable_editor=True, editor_targets=None, generate_images=True, progress_callback=None):
    """Run the content generation pipeline with progress reporting"""
    start_time = time.time()
    results = {
        "metrics": {
            "start_time": start_time,
    ...
    # Step 1: Extract wisdom (common to all pipeline types)
    wisdom = await run_step("Wisdom Extraction", extract_wisdom, transcript, provider, model, knowledge_base)
    results["wisdom"] = {"wisdom": wisdom}
    
    # Different pipeline types
    if pipeline_type == "Social Media Only":
        # Social media only pipeline
        social_content = await run_step("Social Media Content", generate_social_content, wisdom, "", provider, model, knowledge_base)
        results["social"] = {"social_content": social_content, "parsed_social_content": parse_social_content(social_content)}
        
        # Editor pass on social content if enabled
        if enable_editor and "Social Media" in editor_targets:
            editor_result = await run_step("Editor: Social Media", editor_enhancement, social_content, "social", provider, model, knowledge_base)
            results["editor_social"] = {
                "edited_social_content": editor_result["edited_content"],
                "editor_notes": editor_result["notes"],
                "parsed_social_content": parse_social_content(editor_result["edited_content"])
        ...
        # Minimal pipeline - wisdom + outline
        outline = await run_step("Content Outline", generate_outline, transcript, wisdom, provider, model, knowledge_base)
        results["outline"] = {"outline": outline}
        
        # Editor pass on outline if enabled
        if enable_editor and "Outline" in editor_targets:
            editor_result = await run_step("Editor: Outline", editor_enhancement, outline, "outline", provider, model, knowledge_base)
            results["editor_outline"] = {
                "edited_outline": editor_result["edited_content"],
                "editor_notes": editor_result["notes"]
            }
        ...
        social_content = await run_step("Social Media Content", generate_social_content, wisdom, working_outline, provider, model, knowledge_base)
        results["social"] = {"social_content": social_content, "parsed_social_content": parse_social_content(social_content)}
        ...
        # Generate image prompts if enabled
        if generate_images:
            image_prompts = await run_step("Image Prompts", generate_image_prompts, wisdom, working_outline, "", provider, model, knowledge_base)
            results["image_prompts"] = {"prompts": image_prompts}
        ...
        # Generate blog post using the outline (original or edited)
        blog_post = await run_step("Blog Post", generate_article, transcript, wisdom, working_outline, provider, model, knowledge_base)
        results["blog"] = {"blog_post": blog_post, "blog_title": extract_title(working_outline)}
        
        # Editor pass on blog post if enabled
        if enable_editor and "Blog Post" in editor_targets:
            editor_result = await run_step("Editor: Blog Post", editor_enhancement, blog_post, "blog", provider, model, knowledge_base)
            results["editor_blog"] = {
                "edited_blog_post": editor_result["edited_content"],
                "editor_notes": editor_result["notes"]
            }
        ...
        # Generate image prompts if enabled
        if generate_images:
            image_prompts = await run_step("Image Prompts", generate_image_prompts, wisdom, working_outline, working_blog, provider, model, knowledge_base)
            results["image_prompts"] = {"prompts": image_prompts}
    ...
    if "outline" in results and "outline" in results["outline"]:
        ...
        summary = await run_step("Content Summary", generate_summary, wisdom, working_outline, provider, model)
        results["summary"] = {"summary": summary}
        
        tags = await run_step("Content Tags", generate_tags, wisdom, working_outline, provider, model)
        results["tags"] = {"tags": tags}
