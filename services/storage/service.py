...
async def store_transcription(request: StorageRequest):
    try:
        ...
        if not database_id:
            raise HTTPException(
                status_code=500, detail="Notion database ID not configured"
            )
        ...
        response = notion.pages.create(
            ...
            properties={
                "Name": {
                    "title": [
                        {"text": {"content": f"Transcription: {request.file_name}"}}
                    ]
                },
                "Tags": {
                    "multi_select": [{"name": "transcription"}, {"name": "Technology"}]
                },
        ...
        return {"status": "success", "url": response.get("url")}
